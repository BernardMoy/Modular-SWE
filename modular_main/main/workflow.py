...
DEMO_Q_WORKSPACE = Path("agent_workspace_demo_question")
DEMO_A_WORKSPACE = Path("agent_workspace_demo_analyser")
...
def _analyzer_refactor_loop(
    ...
    def update_metrics(agent_workspace):
        ...
        if WORKFLOW_MODE != "autoNoMetric":
            ...
            _report_stage(
                stage_callback,
                f"(Iteration {iteration+1}) Generate implementation metrics",
            )
...
def modular_workflow_single(
    ...
    REPORT_DIR = PROBLEM_DIR / f"report_{SUFFIX}"  # agent reports
    ...
    # If settings demo mode is on, run this instead
    if DEMO_MODE:
        _report_stage(stage_callback, "(Iteration 1) Analyser Agent")

        # The normal workflow creates the workspace below, but demo mode
        # returns before reaching that setup. Start from the demo fixture so
        # the UI has the design, dependency graph, and other demo artifacts.
        if AGENT_WORKSPACE.exists():
            shutil.rmtree(AGENT_WORKSPACE)

        # Copy the demo workspace
        shutil.copytree(DEMO_Q_WORKSPACE, AGENT_WORKSPACE)
        ...
        # Write a question to human request json
        with open(AGENT_WORKSPACE / "human_request.json", "w") as f:
            f.write("""{
                "request_id": "bed689efa8c147c1a71626e794b55d28",
                "type": "question",
                "question": "Smell: `segmentation_service`, `normalization_service`, and `encoding_service` each orchestrate dataset workflows, single-item workflows, path handling, anomaly collection, and optional output persistence, creating a repeated service-layer pattern.\n\nSuggestion: Consider extracting only the demonstrably common orchestration mechanics into a shared workflow utility while keeping segmentation, normalization, and encoding policies in their respective services.\n\nTrade-offs: This could reduce coordination and duplicated workflow structure, but it would add an abstraction and could make distinct domain workflows harder to follow. The design alone does not show whether these workflows are intentionally generic or whether their orchestration details are expected to diverge.\n\nAre the dataset/single-item orchestration patterns intended to remain structurally similar across these services, or are their workflows expected to evolve independently?"
                }""")

        # Wait for 60s.
        # Answering human question wont make it advance to the next step
        # As that is handled inside run_agent
        time.sleep(60)

        # Display the analyser suggestions from here
        if AGENT_WORKSPACE.exists():
            shutil.rmtree(AGENT_WORKSPACE)
        shutil.copytree(DEMO_A_WORKSPACE, AGENT_WORKSPACE)
        _report_analyzer_suggestions(
            analyzer_suggestions_callback,
            get_formatted_analyzer_suggestions(AGENT_WORKSPACE),
        )
    ...
    if not logged_in:
        if SIGN_IN_METHOD == "chatgpt":
            ...
            _report_stage(
                stage_callback, f"Coding agent sign in (Double click)\n{link}"
            )
    ...
    if WORKFLOW_MODE == "noDesign":
        ...
        if WORKFLOW_MODE == "autoByLayer":
            for i, layer in enumerate(modules_array):
                ...
                output = get_prompt_and_run_agent(executor, "modular_coder", N, layer)
                # reflect in the UI the agent output
                _report_agent_response(agent_response_callback, output)
        else:
...
def modular_workflow():
    ...
    if UI_ENABLED:
        ...
        agent_workspace = Path(f"agent_workspace_{args.problem_name}_{start}_{SUFFIX}")
        run_ui(
