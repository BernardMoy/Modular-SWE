...
from textual.widgets import Button, Footer, Header, Input, TextArea, Label, ListItem, ListView
...
design_data = {
    "module_1": "Content1 " + lorem, 
    "module_2": "Content2 " + lorem 
}
impl_data = {
    "file_1": "Content1 " + lorem, 
    "package.file_2": "Content2 " + lorem 
}
...
class TitleApp(App[None]):
    ...
    def __init__(self, workspace: Path | None = None) -> None:
        ...
        self.workspace = workspace or Path("agent_workspace")  # workspace is the agent workspace path
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="stage"):
            yield Label("Stage: Initial decomposer agent", id="stage-label")
        ...
        with Container(id="main-content"):
            ...
            with Vertical(classes="panel") as files_panel:
                ...
                with ListView(id="design-list", initial_index=None):  # Disable the initial selection highlighting
                    ...
                    with Vertical(id="suggestions"):
                        for index, suggestion in enumerate(self.analyzer_suggestions_data):
                            with Horizontal(classes="suggestion-row"):
                                ...
                                yield Label(str(suggestion.get("module_name", "Unknown")))
                                yield Label(str(suggestion.get("description", "Unknown")))

                                # Multi line user input for feedback 
                                yield TextArea(id=f"feedback-{index}")

                    # The submit button: Only available in the human mode 
                    # Else, it is disabled 
                    with Horizontal(id="submit-button"):
                        yield Button("Submit", variant="success", id="submit", disabled=False)
...
def run_ui(workspace: Path | None = None) -> None:
    """Run the UI skeleton for the given agent workspace."""
    TitleApp(workspace=workspace).run()
