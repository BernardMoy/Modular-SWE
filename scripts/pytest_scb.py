...
def pytest_scb(problem_name, entrypoint_path, checkpoint_number_to_test_against): 
    SOLS_TESTS_DIR = Path(f"datasets/slopCodeBench/scb-problems-sols-tests/{problem_name}")
    TEST_SOURCE = f"{SOLS_TESTS_DIR}/tests/test_checkpoint_{checkpoint_number_to_test_against}.py"
    TEST_REPORT_FILE = Path(f"test_report_{problem_name}_{checkpoint_number_to_test_against}.json")

    subprocess.run(
        [
            "uv", "run", "pytest",
            TEST_SOURCE,
            "--entrypoint", f"python {entrypoint_path}",
            "--checkpoint", f"checkpoint_{checkpoint_number_to_test_against}",
            "--json-report",
            "--json-report-file", TEST_REPORT_FILE,
        ],
...
def main(): 
    ...
    parser.add_argument("entrypoint_path", help="abs path to the entrypoint of the scb problem")
    parser.add_argument("checkpoint_no", help="Checkpoint number to test against, can be different from that in the path")
