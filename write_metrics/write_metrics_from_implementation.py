...
def write_metrics_from_implementation(
    implementation_path, current_metrics_path, prev_implementation_path=None
):
    ...
    ) + _get_smells_from_deps_graph(implementation_path)

    # obtain the summary from dpy and jscpd
    dpy_summary = get_dpy_summary(
        dpy_folder_new=TEMP_METRICS_NEW_DIR / "dpy_metrics",
        dpy_folder_old=(
            TEMP_METRICS_OLD_DIR / "dpy_metrics" if prev_implementation_path else None
        ),
    )
    jscpd_summary = get_jscpd_summary(
        implementation_new=implementation_path,
        implementation_old=prev_implementation_path,
    )
    summary = {
        **dpy_summary,
        **jscpd_summary,
    }  # jscpd summary already includes the duplicates data
    ...
    result = {"summary": summary, "smells": smells}
...
def main():
    ...
    parser.add_argument(
        "implementation_path", help="Abs path to the implementation directory"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Abs path to the previous implementation for writing summary",
    )
