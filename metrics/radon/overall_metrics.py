...
def get_radon_metrics(implementation_path):
    ...
    for root, dirs, files in os.walk(implementation_path):
        ...
        dirs[:] = [
            d for d in dirs if d not in {"dist", "node_modules", "__pycache__", ".venv"}
        ]
        for f in files:
            if f.endswith(".py"):
                ...
                try:
                    ...
                    cur_max_lloc = max(cur_max_lloc, analyzed.lloc)
    ...
    return {
        ...
        "weighted_maintainability_index": (
            weighted_mi / total_sloc if total_sloc > 0 else -1
        ),
    }
