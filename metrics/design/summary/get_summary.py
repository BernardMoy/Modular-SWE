...
def get_summary(design_json_new, design_json_old=None):
    ...
    summary["number_of_public_methods_per_module"]["new_value"] = (
        new_total_public_methods / new_total_modules if new_total_modules > 0 else "NA"
    )
    ...
    if design_json_old:
        ...
        summary["number_of_modules"]["change"] = (
            f"+{new_total_modules-old_total_modules}"
            if new_total_modules - old_total_modules > 0
            else str(new_total_modules - old_total_modules)
        )
        summary["number_of_public_methods_per_module"]["change"] = (
            old_total_public_methods / old_total_modules
            - new_total_public_methods / new_total_modules
            if old_total_modules > 0 and new_total_modules > 0
            else "NA"
        )
