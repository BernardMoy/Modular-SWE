...
def get_metrics_summary_from_dpy(dpy_folder_path_old, dpy_folder_path_new=None):
    ...
    if dpy_folder_path_new:
        ...
        for entry in class_json_old:
            ...
            d[key] = {"class": key, "before": entry["LOC"]}
        ...
        for entry in class_json_new:
            ...
            if key in d:
                d[key][
                    "function"
                ] = f"{entry["Module"]}.{entry["Module"]}{'.'+entry["Class"] if entry["Class"] else ""}"
                d[key]["after"] = entry["LOC"]
                d[key]["percentage_change"] = (
                    100 * (d[key]["after"] - d[key]["before"]) / d[key]["before"]
                    if d[key]["before"] > 0
                    else 0
                )

        result["top_increased_lines_of_code_classes"] = sorted(
            [x for x in d.values() if "percentage_change" in x],
        ...
        for entry in class_json_old:
            key = f"{entry["Module"]}{'.'+entry["Class"] if entry["Class"] else ""}"
            d[key] = {"class": key, "before": entry["NOM"]}
        ...
        result["top_increased_number_of_methods_classes"] = sorted(
            [x for x in d.values() if "percentage_change" in x],
            key=lambda x: x["percentage_change"],
            reverse=True,
        )[:TOP_CLASS_THRESHOLD]
    ...
    if dpy_folder_path_new:
        ...
        for entry in function_json_old:
            ...
            d[key] = {"function": key, "before": entry["LOC"]}
        ...
        result["top_increased_lines_of_code_functions"] = sorted(
            [x for x in d.values() if "percentage_change" in x],
            key=lambda x: x["percentage_change"],
            reverse=True,
        )[:TOP_FUNCTION_THRESHOLD]

        d = {}
        for entry in function_json_old:
            key = f"{entry["Module"]}.{entry["Class"] + '.' if entry["Class"] else ""}{entry["Method"]}"
            d[key] = {"function": key, "before": entry["CC"]}
        ...
        result["top_increased_cyclomatic_complexity_functions"] = sorted(
            [x for x in d.values() if "percentage_change" in x],
            key=lambda x: x["percentage_change"],
            reverse=True,
        )[:TOP_FUNCTION_THRESHOLD]
...
def main():
    ...
    summary = get_metrics_summary_from_dpy(args.old_dpy_path, args.new_dpy_path)
